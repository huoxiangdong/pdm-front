...
export default {
  async inserted(el,binding,vnode) {
    ...
    console.log('这里是bind')
      ...
      //  initChild_X = el.getBoundingClientRect().x + borderWidth + window.scrollX,
      //  initChild_Y = el.getBoundingClientRect().y + borderWidth + window.scrollY
    if(el.children.length) {
      ...
      //      initChild_Y = el.children[i].getBoundingClientRect().y
           ...
           function move(e) { // 鼠标移动
            if(!isDown) {
              return 
              // if(e.target.getBoundingClientRect().x == e.pageX) {
              //   console.log(e.target.getBoundingClientRect().x +',' + e.pageX)
              // el.children[i].style.cursor = 'crosshair'
              // }else {
             ...
             let moveX = e.pageX - ((parentRect.x + borderWidth + window.scrollX) + offsetX), // 元素X坐标 鼠标X轴与元素X轴的距离 ,
                 moveY = e.pageY - ((parentRect.y + borderWidth + window.scrollY) + offsetY),
               // 边界判断
